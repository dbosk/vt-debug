...
By now, we fully understand this phenomenon, and we see Python in a more
powerful way.
...
\begin{enumerate}
  ...
  \item That the default value is itself an object with a construction
    time---and construction time and execution time are the same thing:
    execution controls construction, so \emph{when} the definition is
    executed determines when the default is constructed.
\end{enumerate}
...
Nowadays we can also record such sessions using learnlog
...
We are transcribing these; the transcripts will then be analysed with the
episode coding of \cref{method}, and the coded episodes reported here.
