...
\end{restatable}
\begin{restatable}{question}{rqschool}\label{rq:school}
  How do the schools a student attended before the course---their type
  of principal (\emph{huvudman}), and the schools themselves---relate to
  the approach to learning they bring to it, and to how they fare in it?
\end{restatable}
\end{frame}
...
\Cref{rq:school} looks outside the course altogether: the schools Swedish
students attended before it are recorded in a national register, so the
type of principal that ran them, and the schools themselves under
pseudonymous codes, can be related to both the approach measure and the
course outcomes without asking the students to judge anything.
It is exploratory, and we motivate it in \cref{schooling}.
Together this makes for a mixed-methods design, which we detail in
\cref{method}.

Answering the questions requires methods that are themselves untried: we
code patterns of variation from recorded edit--run cycles, and we
classify approaches to learning from several sources, none of which is
the original studies' method.
The study must therefore verify its own instruments, which we state as
methodological questions, separate from the substantive ones.
\begin{frame}
\begin{restatable}{mquestion}{mqcoding}\label{mq:coding}
  Do the patterns of variation coded from the recorded edit--run cycles
  correspond to the patterns in the debugger's reasoning?
\end{restatable}
\begin{restatable}{mquestion}{mqagreement}\label{mq:agreement}
  Do the different classifications of the students' approaches to
  learning---the questionnaire scores, the interview accounts and the
  coded open-text answers---agree?
\end{restatable}
\end{frame}
Each is answered by comparing measurements of the same thing from
independent sources; we detail how in \cref{method}.
...
\begin{itemize}
  \item A variation-theoretic \emph{theory} of debugging: debugging is
    self-directed learning, and debugging skill is the ability to
    generate the patterns of variation for oneself
    (\cref{debugging-as-learning}), illustrated by a worked example of an
    actual debugging episode (\cref{debugging}).
    \Cref{rq:patterns} tests the theory's central prediction.
  \item The identification of debugging skill with the deep approach to
    learning, via \citeauthor{NCOL}'s reinterpretation of the approaches
    (\cref{approaches}); \cref{rq:deep,rq:success} test it.
  \item An expert baseline for debugging: the course's teachers, an AI
    coding agent, and the systematic-debugging literature---where we
    check that the literature's prescriptions are variation theory
    without knowing it (\cref{rq:experts}).
  \item A within-cohort test of generative learning: whether experiencing
    patterns of variation in the course material breeds generating them
    in debugging (\cref{rq:generative}).
  \item An exploratory link between the schooling a student brings and
    the approach to learning they bring with it: the type of principal
    of each school they attended, taken from the national school
    register rather than from self-report, and the schools themselves
    under pseudonymous codes, related to the approach measure and to the
    course outcomes (\cref{rq:school}).
  \item A method for observing patterns of variation in programming
    process data: learnlog's recorded edit--run cycles and a
    variation-theoretic coding of them, validated against think-aloud
    reasoning (\cref{mq:coding}).
  \item An approach classification compatible with the original studies'
    phenomenographic one, triangulated from questionnaire, interview and
    open-text sources, with the agreement between them reported
    (\cref{mq:agreement}).
\end{itemize}
...
\begin{frame}
  \begin{block}{Contributions}
    \begin{itemize}
      \item Theory: debugging \(=\) self-directed variation.
      \item Debugging skill \(=\) the deep approach.
      \item Expert baselines: teachers, AI agent, the literature.
      \item Generative learning, tested within the cohort.
      \item Exploratory: the schools they came from, from the register.
      \item Method: coding variation from edit--run logs (validated).
      \item Method: triangulated approach classification (agreement).
    \end{itemize}
  \end{block}
\end{frame}%
}
...
the comparison with our design is direct---in \cref{discussion} we discuss
implications and threats to validity, and in \cref{conclusions} we
conclude.
The appendices contain the study's instruments as literate programs---the
diagnostic quizzes (\cref{app:quiz}), the R-SPQ-2F surveys
(\cref{app:rspq}) and the episode analysis
(\cref{app:episodes})---and the search protocol behind the references
(\cref{app:search}).
