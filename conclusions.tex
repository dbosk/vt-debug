...
\rqschool*
Nothing yet, and by design the least we will be able to say: the schools
the students came from are collected with the background quiz and their
principals looked up in the register (\cref{school-lookup}), but the
mechanism \cref{h:school} names is our conjecture rather than a finding
of the literature (\cref{schooling}), and the huvudman type is a weak
covariate in a single course.
What the prgi26 data can give is a first look at whether the association
is there at all.

The methodological questions also await the data, but the pilot bears on
the first.
\mqcoding*
In the pilot (\cref{pilot}) the patterns were identifiable in think-aloud
recordings; whether they can be coded from the edit--run cycles alone is
what the think-aloud subsample will verify (\cref{think-aloud}).
\mqagreement*
Entirely pending the data: the three classifications---questionnaire,
interview accounts, open-text answers---are collected precisely so that
their agreement can be reported (\cref{approach-measure}).
