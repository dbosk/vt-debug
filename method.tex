...
The data collection maps onto the research questions as follows.
The learnlog recordings (\cref{learnlog-data}) yield the debugging episodes
in which we look for the patterns of variation (\cref{rq:patterns})---for
the students and for the teachers alike, the latter giving the expert
baseline (\cref{rq:experts}, \cref{teacher-baseline}).
The questionnaire (\cref{approach-measure}) provides the approach measure
that we relate to the generated patterns (\cref{rq:deep}), and the
debugging outcomes (\cref{outcomes}) the success measure
(\cref{rq:success}).
The diagnostic quiz (\cref{prior-knowledge}) and the material use
(\cref{material-use}) provide the control variables and the within-cohort
comparison groups.
...
(\cref{learnlog-data,material-use,teacher-baseline}), and what derives from
the recordings afterwards (\cref{outcomes}).
...
\begin{enumerate}
  \item Prior debugging habits, through an open question such as
    ...
    logs and the questionnaire.
  \item Languages known and years of experience, and whether they know
    Python specifically.
  \item How they learned to program: in a formal course or self-taught---
    the self-taught being self-directed learners by definition.
  \item Programming self-efficacy, which confounds persistence measures
    such as the time and the number of cycles to fix.
  \item How they plan to study in this course, as a baseline for the
    material-use self-report (\cref{material-use}).
\end{enumerate}
