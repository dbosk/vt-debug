...
stay fresh.
For the knowledge test we use questions from the course's example
exam---as many as fit alongside the approach questionnaire (the
R-SPQ-2F, which we introduce in \cref{approach-measure}) and the
background quiz---improved where needed and translated into
English.
This measures what we want to measure: the exam questions are
constructed against the course's intended learning outcomes, so the
knowledge scores at the start, and again at the end
(\cref{end-measure}), measure prior knowledge and knowledge gain in
terms of what the course intends to teach.
It also keeps the study transparent: the questions can be included in
full in this paper (\cref{app:quiz}).
The alternative---an established concept inventory, which would buy
comparability with other studies at the price of both of these---we
discuss in \cref{inventory-discussion}.
