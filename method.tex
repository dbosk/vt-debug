...
stay fresh.
The questions of the knowledge test are our own, written for this study
against the course's intended learning outcomes---one per outcome area,
as many as fit alongside the approach questionnaire (the R-SPQ-2F,
which we introduce in \cref{approach-measure}) and the background quiz.
...
And being our own, they can be published in full (\cref{app:quiz}): the
course's existing tests---its example exam and the earlier instances'
diagnostic tests, which these quizzes replace---are their authors'
copyright, so reusing their questions would have kept the instrument
unpublishable.
