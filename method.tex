...
The data collection maps onto the research questions as follows; we
restate the questions and hypotheses to give the overview here, the
details follow in the coming sections.
\begin{description}
  \item[\Cref{rq:patterns}] \enquote{\rqpatterns}
    \Cref{h:patterns} predicts the answer: \enquote{\hpatterns}
    We answer this by coding the debugging episodes recorded with learnlog
    (\cref{learnlog-data}) in variation-theoretic terms (\cref{analysis}).
  \item[\Cref{rq:deep}] \enquote{\rqdeep}
    \Cref{h:deep} predicts: \enquote{\hdeep}
    The questionnaire (\cref{approach-measure}) provides the approach
    measure that we relate to the coded patterns.
  \item[\Cref{rq:success}] \enquote{\rqsuccess}
    \Cref{h:success} predicts: \enquote{\hsuccess}
    The debugging outcomes (\cref{outcomes}) provide the success measures.
  \item[\Cref{rq:experts}] \enquote{\rqexperts}
    The teachers' and assistants' recordings give the expert baseline
    (\cref{teacher-baseline}); this comparison is exploratory
    (\cref{hypotheses}).
\end{description}
...
session with learnlog (\cref{learnlog-data}) together with the audio.
