...
\begin{description}
  ...
  \item[\Cref{rq:experts}] \enquote{\rqexperts}
    ...
    (\cref{hypotheses}).
  \item[\Cref{rq:generative}] \enquote{\rqgenerative}
    \Cref{h:generative} predicts: \enquote{\hgenerative}
    The material-use measure (\cref{material-use}) identifies the groups;
    the comparison controls for approach (\cref{approach-measure}) and
    prior experience (\cref{prior-knowledge}).
\end{description}
The diagnostic quiz (\cref{prior-knowledge}) provides the control
variables; together with the material use (\cref{material-use}) it also
defines the within-cohort comparison groups.
